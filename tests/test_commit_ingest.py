"""Tests for GraphStore commit-intent ingest interface (Task 2).

These tests exercise the commit node CRUD + reverse/forward queries that
``GraphStore`` exposes on top of the ``commits`` + ``commit_modifies_file``
tables added by migration v10.
